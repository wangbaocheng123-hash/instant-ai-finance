...
export const instantApi = {
  ...
  saveBloggerTitle: (workKey: string, title: string) => request<{ ok: boolean; title: string; saved: boolean; mode: string }>(`/api/blogger-library/works/${encodeURIComponent(workKey)}/title`, {
    method: 'POST',
    body: JSON.stringify({ title }),
  }),
  saveBloggerVideoText: (workKey: string, text: string) => request<{ ok: boolean; text: string; saved: boolean; mode: string }>(`/api/blogger-library/works/${encodeURIComponent(workKey)}/video-text`, {
    method: 'POST',
    body: JSON.stringify({ text }),
  }),
  transcribeBloggerWork: (workKey: string, engine: 'video' | 'doubao') => request<ModelMrTranscriptionResult>(`/api/blogger-library/works/${encodeURIComponent(workKey)}/${engine === 'doubao' ? 'doubao-transcribe' : 'transcribe'}`, {
    method: 'POST',
    body: JSON.stringify({}),
  }),
