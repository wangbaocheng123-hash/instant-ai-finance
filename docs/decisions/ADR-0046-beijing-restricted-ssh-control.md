...
- 状态：ACCEPTED / SSH_PUBLISH_LIVE_VERIFIED（手机独立及回滚演练尚未完成）
- 日期：2026-09-10
- 17:48结果更新：北京Git低速超时经可信日志确认、直连预检恢复后，新加坡受限客户端受控重试一次，正式发布d60f75f并两次精确回执验收成功；北京220项隔离测试通过，模型下载器未重启，timer关闭。main整合和新Codex CLI任务已完成。北京正式HTTPS与回环通过，新加坡直接HTTPS reset另行待查，未修改网络/基础设施。详细证据见运行手册顶部，以下17:32为历史。
- 17:32结果更新：main整合和真实新加坡Codex CLI新任务只读验证已完成；用户授权的首次发布已触发但北京发布服务退出错误，旧生产保持健康。未出现成功回执，手机关机电脑实机验收和真实回滚演练未做。详细版本及阻塞见运行手册最新节，不能套用下方首次启用阶段的未整合描述。
- 授权：所有者批准最终方案后要求“开始实施”。不是生产业务发布授权。
- 替代范围：ADR-0045 的当前北京控制传输选择；双组件隔离、源码核验、数据保护继续有效。

2026-09-10后续授权：所有者明确要求完成main整合和独立验证后直接发布北京博主采集器，无需再次确认。该次授权替代本文首次启用阶段的“本轮不发布”，仅对审查和测试通过的北京组件生效；不会运行新加坡即时AI发布器或更改基础设施。物理手机/电脑关机测试仍必须与服务器进程验证分开记账。

## 事实与原因

可信 Alibaba Cloud Client 显示，北京 `47.93.214.76` 是轻量应用服务器（SWAS），
并非 ECS。实例为 `e905b859a6c24c529ef5c2ea21658462`。后续可信控制台核实北京账号 ID 为
`1051070265710978`，新加坡也为 SWAS（`c9e012a3ed944b588d696e71c0e24aea`）。
不能把新加坡账号 ID 填入北京策略，也不能把 ECS 的 `CommandRunAs` 约束直接套到 SWAS。
既有 CLI、模板、Git 发布器和只读桥保留，不以扩大 RAM 权限解决产品差异。

现场限定例外：所有者随后明确批准改用已登录内置浏览器 Workbench 执行本次两台服务器的首次启用，并单独确认只新增北京 TCP22 来源 `47.236.175.118/32`。该规则已增加，原六条保留；不构成更多网络改动或任意云操作授权。私钥仅在新加坡生成，详细完成/未完成状态以 `docs/BEIJING_RESTRICTED_SSH_CONTROL.md` 的现场检查点为准。
