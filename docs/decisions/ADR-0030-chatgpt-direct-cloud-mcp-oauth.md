...
13. 0.18.5 起，callback-ID 模式在主人凭据验证成功后不再仅依赖浏览器自动处理 302，而是显示一次性的“返回 ChatGPT 完成授权”链接，由主人主动完成官方回调。链接不显示、不记录、不可缓存，仍携带原始 state 和五分钟一次性授权码；稳定回调客户端继续使用原 302，不改变既有兼容契约。
...
- 0.18.5 回归覆盖 callback-ID 成功页、主动回调链接、已有主人会话免重复密码、后续 token/工具调用，以及旧稳定回调仍返回 302 与 `iss`；完整 Python 回归增至 96 项。
