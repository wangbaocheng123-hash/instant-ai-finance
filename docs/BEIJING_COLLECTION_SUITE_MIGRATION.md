...
> 2026-09-10 更正：当前 `47.93.214.76` 是 SWAS 轻量应用服务器，不是 ECS，下列旧阶段 A
> 的 ECS RAM 步骤只保留为历史，不能用于该目标。
>
> 2026-09-10 完成更新：ADR-0047 已替代日常受限入口。新加坡 `beijing-admin` 对北京
> 拥有完整管理员能力；现场真实模型下载器已导入、测试并由统一发布器上线。以下阶段 A
> 至 C 保留为迁移历史，不再表示当前阻塞。
