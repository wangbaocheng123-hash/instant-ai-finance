...
0.23.0 已正式发布：北京模型先生下载器可同时识别视频与图文，图文会保存抖音公开正文和按发布顺序排列的全部原图，经既有签名通道投影到主人登录后的手机原图库；MCP 只读元数据可看到作品类型、图片数和正文。图文不会进入视频 ASR/自动处理队列，图片像素文字也不会被自动 OCR 后混入公开正文。当前生产库尚无自然图文，真实样本验收等待模型先生下一条 `/note/`，不以合成测试代替。
