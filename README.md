...
0.23.2 候选版统一模型先生的北京时间契约：北京下载器给作品和评论时间写入明确的 `+08:00`，传输桥将历史无偏移值按 `Asia/Shanghai` 解释，手机页面也固定按北京时间显示。它修复了真实 `9月23日17:59` 被重复加八小时显示为 `9月24日01:59` 的问题；历史校时修订复用已有视频内容对象，不重复上传 MP4。本版本尚未正式发布。
