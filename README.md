...
`模型先生`作为独立的单主人手机资料库位于“重点事件关注”右侧。登录后可直接播放有声压缩视频、查看或保存视频原文、读取普通/豆包识别结果、浏览评论、投资思路和受控问答；抖音原地址只作来源核验。统一云端 MCP 的普通作品文字工具不返回评论；0.21.0 已正式增加窄范围作者回复工具，只读取来源明确标记的本人发言及对应原提问，不按昵称推断、不返回整片评论区。0.21.4 已正式把新视频改为送达即自动识别原文并提炼 AI 关键词，回调漏失可由完成账本补偿；每日额度、单视频时长、串行锁、异常核对和已有资料不覆盖仍是硬边界。代码与业务资料严格分离：视频、评论和详情存放在 Git 外主人目录，原项目的粉丝资料、管理功能、整份 `.env`、密钥值、原始数据库、本机路径和原始 JSON 都不进入本仓库或公网接口。
