...
`模型先生`作为独立的单主人手机资料库位于“重点事件关注”右侧。登录后可直接播放有声压缩视频、查看或保存视频原文、读取普通/豆包识别结果、浏览评论、投资思路和受控问答；抖音原地址只作来源核验。统一云端 MCP 的普通作品文字工具不返回评论；0.21.0 候选另增窄范围作者回复工具，只读取来源明确标记的本人发言及对应原提问，不按昵称推断、不返回整片评论区。可选现场豆包语音识别必须由所有者另行批准，以 Git 外 root 专用凭据启用，并在每次付费调用前确认。代码与业务资料严格分离：视频、评论和详情存放在 Git 外主人目录，原项目的粉丝资料、管理功能、整份 `.env`、密钥值、原始数据库、本机路径和原始 JSON 都不进入本仓库或公网接口。

`博主`是即时 AI 现有仓库内的单主人模块，入口位于模型先生右侧。北京服务器保留原模型下载器并旁路运行独立博主采集服务；采集到的作品、公开评论、作者互动和基础分类通过签名 HTTPS 主动推送到新加坡。新加坡直接复用即时 AI 已有主人登录、HTTPS 和发布链路，但博主资料使用独立 Git 外目录与 SQLite，不混入财经新闻或模型先生数据。0.21.0 候选把普通博主的作品视频、原文、作者红色标识、评论排序、确定性评股、AI 关键词和解读感悟与模型先生对齐，但不增加投资思路或智能问答。可选的新视频自动识别/关键词队列默认关闭、不扫描历史，只有主人确认费用并开启后才处理随后到达的作品。
