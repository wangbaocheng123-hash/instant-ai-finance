...
`博主`是即时 AI 现有仓库内的单主人模块，入口位于模型先生右侧。北京服务器保留原模型下载器并旁路运行独立博主采集服务；采集到的作品、公开评论、作者互动和基础分类通过签名 HTTPS 主动推送到新加坡。新加坡直接复用即时 AI 已有主人登录、HTTPS 和发布链路，但博主资料使用独立 Git 外目录与 SQLite，不混入财经新闻或模型先生数据。0.21.0 已正式把普通博主的作品视频、原文、作者红色标识、评论排序、确定性评股、AI 关键词和解读感悟与模型先生对齐，但不增加投资思路或智能问答。可选的新视频自动识别/关键词队列默认关闭、不扫描历史，只有主人确认费用并开启后才处理随后到达的作品。
