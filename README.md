...
0.23.0 当前是尚未正式发布的开发候选：北京模型先生下载器可同时识别视频与图文，图文会保存抖音公开正文和按发布顺序排列的全部原图，经既有签名通道投影到主人登录后的手机原图库；MCP 只读元数据可看到作品类型、图片数和正文。图文不会进入视频 ASR/自动处理队列，图片像素文字也不会被自动 OCR 后混入公开正文。线上仍保持 0.22.1，正式发布须由所有者另行明确要求。
