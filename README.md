...
`模型先生`作为独立的单主人手机资料库位于“重点事件关注”右侧。登录后可直接播放有声压缩视频、查看或保存视频原文、读取普通/豆包识别结果、浏览评论、投资思路和受控问答；抖音原地址只作来源核验。代码与业务资料严格分离：视频、评论和详情存放在 Git 外主人目录，原项目的粉丝资料、管理功能、`.env`、密钥、原始数据库、本机路径和原始 JSON 都不进入本仓库或公网接口。
