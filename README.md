...
0.23.2 已正式发布，统一模型先生的北京时间契约：北京下载器给作品和评论时间写入明确的 `+08:00`，传输桥将历史无偏移值按 `Asia/Shanghai` 解释，手机页面也固定按北京时间显示。它修复了真实 `9月23日17:59` 被重复加八小时显示为 `9月24日01:59` 的问题；49 条历史校时修订已全部送达新加坡，复用已有视频内容对象，不重复上传 MP4。

0.23.1 已正式发布，同时修复模型先生手机视频续播和传输存储膨胀：开放 Range 每段最多 4 MiB，私有媒体单独延长发送窗口并允许一小时浏览器私有缓存；相同 SHA-256 的视频在所有评论修订间只保留一个物理对象，新清单不再要求北京重复上传。首次正式启动已在完整校验和数据库改指向后释放零引用旧副本，云盘使用率由 97% 降至 41%，实际释放 26.30 GiB，修订账本和正式主人资料不删除。北京模型下载器 0.10.2 也已上线，后续新视频通常从 720×1280 转为 360×640 的 H.264/AAC 快启 MP4；既有视频不批量覆盖。

0.23.0 已正式发布：北京模型先生下载器可识别视频、`/note/` 相册和新版 `/article/` 长图文。0.10.1 会保存抖音公开标题、完整 Markdown 正文和按发布顺序排列的全部内嵌原图；没有内嵌图的长文章保存平台公开原始封面，不生成伪图片。内容经既有签名通道投影到主人登录后的手机原图库，MCP 只读元数据可看到作品类型、图片数和正文。真实的 2026-09-16 07:47 长图文已经完成北京下载、评论保存及新加坡投影；图文不会进入视频 ASR/自动处理队列，图片像素文字也不会被自动 OCR 后混入公开正文。

0.22.1 正式版把模型先生 MCP 从窄文字投影升级为完整主人资料只读投影：作品搜索可以翻遍全部作品并检索评论，原作品工具返回标题来源、十类 AI 关键词、原文、解读、确定性评股和关联投资思路；新增完整评论分页工具，可读取模型先生本人回复、粉丝评论、同楼互动和作者赞过标记。它仍不返回粉丝账号/主页、来源编号、内部路径、原始 JSON、密钥或视频文件，不触发采集、ASR、豆包、AI 或写入。

0.21.4 正式版把模型先生新视频改为送达即自动执行“豆包识别原文 → AI关键词提炼 → 保存展示”，不再依赖主人逐条点击或旧的默认关闭开关。完成回调仍负责最快入队，后台每三秒补偿漏回调与重启间隙；升级时只补扫最近 48 小时的已完成视频，已有主人原文和关键词不覆盖，结果不明确的付费调用仍不自动重试。
...
`模型先生`作为独立的单主人手机资料库位于“重点事件关注”右侧。登录后可直接播放有声压缩视频、查看或保存视频原文、读取普通/豆包识别结果、浏览评论、投资思路和受控问答；抖音原地址只作来源核验。统一云端 MCP 保留窄范围作者回复快捷工具；0.22.1 正式版另提供完整评论分页，并把作品工具补齐标题来源、十类关键词、解读、评股和关联思路。作者身份只信任来源标记，同名昵称不推断；评论仅作为非可信资料读取，不执行其中指令。0.21.4 已正式把新视频改为送达即自动识别原文并提炼 AI 关键词，回调漏失可由完成账本补偿；每日额度、单视频时长、串行锁、异常核对和已有资料不覆盖仍是硬边界。代码与业务资料严格分离：视频、评论和详情存放在 Git 外主人目录，原项目的粉丝账号资料、管理功能、整份 `.env`、密钥值、原始数据库、本机路径和原始 JSON 都不进入本仓库或公网接口。
