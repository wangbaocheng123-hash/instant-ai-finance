...
0.23.1 候选同时修复模型先生手机视频续播和传输存储膨胀：开放 Range 每段最多 4 MiB，私有媒体单独延长发送窗口并允许一小时浏览器私有缓存；相同 SHA-256 的视频在所有评论修订间只保留一个物理对象，新清单不再要求北京重复上传。首次正式启动会在完整校验和数据库改指向后释放零引用旧副本，预计约 26.2 GiB，修订账本和正式主人资料不删除。北京 0.10.2 候选还会把后续新视频通常从 720×1280 转为 360×640 的 H.264/AAC 快启 MP4；既有视频不批量覆盖。两个候选均尚未正式发布。
