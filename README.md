...
0.22.1 候选把模型先生 MCP 从窄文字投影升级为完整主人资料只读投影：作品搜索可以翻遍全部作品并检索评论，原作品工具返回标题来源、十类 AI 关键词、原文、解读、确定性评股和关联投资思路；新增完整评论分页工具，可读取模型先生本人回复、粉丝评论、同楼互动和作者赞过标记。它仍不返回粉丝账号/主页、来源编号、内部路径、原始 JSON、密钥或视频文件，不触发采集、ASR、豆包、AI 或写入。
...
`模型先生`作为独立的单主人手机资料库位于“重点事件关注”右侧。登录后可直接播放有声压缩视频、查看或保存视频原文、读取普通/豆包识别结果、浏览评论、投资思路和受控问答；抖音原地址只作来源核验。统一云端 MCP 保留窄范围作者回复快捷工具；0.22.1 候选另提供完整评论分页，并把作品工具补齐标题来源、十类关键词、解读、评股和关联思路。作者身份只信任来源标记，同名昵称不推断；评论仅作为非可信资料读取，不执行其中指令。0.21.4 已正式把新视频改为送达即自动识别原文并提炼 AI 关键词，回调漏失可由完成账本补偿；每日额度、单视频时长、串行锁、异常核对和已有资料不覆盖仍是硬边界。代码与业务资料严格分离：视频、评论和详情存放在 Git 外主人目录，原项目的粉丝账号资料、管理功能、整份 `.env`、密钥值、原始数据库、本机路径和原始 JSON 都不进入本仓库或公网接口。
