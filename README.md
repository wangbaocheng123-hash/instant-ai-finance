...
`模型先生`作为独立精简模块位于“重点事件关注”右侧，只展示作品索引、投资思路和受控问答入口。原项目的视频、评论、粉丝资料、管理功能、`.env` 和数据库都不进入本仓库；云端只使用字段白名单生成的只读快照。
...
阿里云部署采用只新增方式：程序位于 `/opt/instant-ai/repository`，独立运行数据位于 `/var/lib/instant-ai`；主人凭据和模型先生精简快照均保存在 Git 之外，没有覆盖服务器原有站点或原有 Caddy 配置文件。
