...
| P2-MODEL-05 | IN_PROGRESS | 把模型先生电脑端核心能力改成即时 AI 单主人手机版并同步压缩视频 | ADR-0022；0.14.0 正式 TypeScript 源码、Vite 构建和 Python 网关均已实现登录后本地视频、Range 播放、正式原文保存、普通/豆包识别结果和评论分页；新增可选现场豆包 ASR、Git 外 root 凭据配置和点击前计费确认，33 项测试通过，凭据尚未获单独批准配置。已推送功能提交 `9c380a2`，后续现场 ASR 补丁待再次推送，媒体与业务数据不入 Git。Git 外资料库含 398 作品、67 转写、33,308 评论、61 思路，384 条作品准确匹配视频，388 个约 1.335GiB 媒体均待同步。云端等待用户在 Alibaba Cloud Client 完成连接并明确正式发布 |
