...
| P3-CLOUD-02 | BLOCKED | 通过桌面 Alibaba Cloud Client 把云端从 0.9.0 增量更新到 0.9.1 | 正式/应急地址当前均健康返回 0.9.0；客户端窗口连续两次读取失败：`SetIsBorderRequired failed (0x80004002)`；按永久规则未回退网页控制台，按 Computer Use 安全规则未自动操作远程终端或使用客户端外 SSH |
