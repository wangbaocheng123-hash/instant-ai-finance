...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布并启用 ChatGPT 直连新加坡博主 MCP | 0.18.2 已正式上线；真实页确认当前服务器带入账号 `amyy088`，受控错误凭据 POST 正常，但再次真实点击仍无可见完成结果。0.18.3 本地候选加入仅回环可读、内存 64 条上限的匿名 OAuth 阶段诊断，可区分授权页、凭据拒绝、302 回调和 token 交换，且不记录 URL、IP、账号、密码、code/state/PKCE/Cookie/token 或业务内容；完整 94 项 Python 回归、Vite 生产构建及 npm 零漏洞审计通过，待明确发布后只重试一次并读取阶段结果 |
