...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布并启用 ChatGPT 直连新加坡博主 MCP | 0.18.5 已正式上线：callback-ID 授权成功后显示“返回 ChatGPT 完成授权”按钮，让主人主动完成回调；旧稳定客户端仍保留 302 与 `iss`。生产仓库为 `26ecf0c`，服务器 96 项回归、健康/Service Worker/MCP 版本、OAuth 元数据、PKCE、服务状态及诊断隔离均通过。下一步删除失败连接并新建一次，点击二次按钮后确认 token 签发和两个只读工具可用 |
