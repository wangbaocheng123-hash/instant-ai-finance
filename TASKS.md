...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布并启用 ChatGPT 直连新加坡博主 MCP | 0.18.4 已正式上线且真实新连接确认 DCR、callback-ID 授权页、主人凭据与服务端授权码生成均成功，但浏览器未完成 302 回调且没有 token 请求。0.18.5 候选改为验证成功后显示“返回 ChatGPT 完成授权”按钮，让主人主动完成 callback-ID 回调；旧稳定客户端仍保留 302 与 `iss`。完整 96 项 Python 回归、前端构建及零漏洞审计通过，待正式发布后删除失败连接并新建一次做真实 token/工具验收 |
