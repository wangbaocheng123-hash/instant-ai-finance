...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布并启用 ChatGPT 直连新加坡博主 MCP | 0.18.3 已正式上线：生产提交 `0253813`，服务器 94 项测试、正式健康、Service Worker、OAuth/MCP、公网诊断 404 与服务状态均通过。匿名阶段快照仅回环可读且不记录 URL、IP、账号、密码、code/state/PKCE/Cookie/token 或业务内容；当前为空。下一步由所有者只点击一次确认授权，随即读取阶段结果并完成连接验收 |
