...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布并启用 ChatGPT 直连新加坡博主 MCP | 0.18.4 callback-ID 兼容修复已正式上线：生产提交 `6ef35e9`，服务器 95 项回归、正式健康/Service Worker/MCP/OAuth、服务状态及诊断隔离通过。0.18.3 已证明主人凭据成功且稳定回调后无 token；新连接将使用独有 callback-ID，旧稳定客户端继续兼容。下一步删除失败外壳、新建一次连接并验收 token/tool call；不得复用旧授权页 |
