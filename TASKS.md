...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布 0.18.0 并启用 ChatGPT 直连新加坡博主 MCP | 生产已由受限发布器从 `800af50` 快进到 `578cd56`，服务器 91 项测试通过；公网 OAuth/MCP 发现、两个只读工具、401 challenge、服务状态及李爱琳rene最新 939 字 official 原文均已真实验收。剩余项仅为 ChatGPT 账号内添加 `https://grandpaamu.com/mcp` 并完成一次主人 OAuth 授权；本机浏览器组件缺失导致本轮无法自动完成网页连接，不得读取 Cookie、密码或改用 API Key |
...
| P2-WATCH-05 | IN_PROGRESS | 补齐 2026 年 9 月美国宏观重点事件，并修通即时 AI→Codex→罗盘报告→手机通知链路 | ADR-0031；ADP、褐皮书、零售销售监测已随 0.18.0 上线。0.18.1/schema 10 候选新增罗盘处理反馈和最近变化置顶；罗盘候选把临时核验改成五分钟、最多三次的 `retry`，保存跳过原因并恢复 72 小时内旧无原因跳过。即时 AI 92 项测试、生产构建和罗盘完整检查通过；两个仓库待分别正式发布并做 ADP 真实恢复验收 |
