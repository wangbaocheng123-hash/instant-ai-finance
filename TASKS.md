...
| P2-MODEL-11 | DONE | 让北京模型下载器作品复用博主通道进入新加坡模型先生资料库 | ADR-0033；北京 1.0.5 只读桥把原下载器完成视频/评论写入现有 outbox，不改下载器、不启动 Chromium；新加坡 0.19.1 在 artifact complete 后按保留身份幂等投影到独立 model-mr 根，普通博主查询排除该身份，现有模型先生 MCP 自动可查。即时 AI 104 项、北京 202 项测试、Vite 构建和 npm 零漏洞审计通过，尚未正式发布或传真实数据 |
| P3-MODEL-02 | NOT_STARTED | 正式发布 0.19.1 并启用北京模型下载器传输桥 | 必须先发布新加坡接收/投影端，再更新北京 1.0.5 与 systemd 补充组；只做一条脱敏端到端验收，确认视频、评论、模型先生页面和现有 MCP 可见，不自动触发 ASR、豆包或 AI |
