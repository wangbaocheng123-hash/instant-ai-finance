...
| P3-CLOUD-02 | BLOCKED | 通过桌面 Alibaba Cloud Client 把云端从 0.9.0 增量更新到 0.9.1 | 用户重新打开客户端后仍只有唯一正确窗口，但可访问性文字为空且截图连续两次报 `SetIsBorderRequired failed (0x80004002)`；正式/应急地址健康返回 0.9.0，第 52 次自动采集为 20 来源、655 条、新增 54、错误 0；未回退网页控制台、未猜测点击、未自动操作远程终端或使用客户端外 SSH |
