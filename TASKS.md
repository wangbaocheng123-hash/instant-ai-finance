...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布并启用 ChatGPT 直连新加坡博主 MCP | 0.18.3 真实诊断确认 DCR、授权页、主人凭据和 302 均成功，稳定回调连续发出三次但从未进入 token 交换，断点在 OpenAI 客户端回调层。0.18.4 候选改让新连接使用独有 callback-ID URI，旧稳定客户端保持兼容；匿名诊断可区分两种模式，完整 95 项 Python、Vite 构建及 npm 零漏洞审计通过。待明确发布后删除失败外壳、新建一次连接并验收 token/tool call |
