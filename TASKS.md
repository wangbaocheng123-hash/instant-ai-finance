...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布并启用 ChatGPT 直连新加坡博主 MCP | 0.18.0 生产 OAuth 已真实复现授权后循环：插件外壳创建成功但令牌交换未完成。0.18.1 本地候选修复 DCR 按稳定回调复用旧 `client_id` 的问题，改为每个 connector instance 独立客户端并自动迁移旧库；授权码只在完整校验成功后消费，运行挑战补齐错误语义。针对性 7 项及隔离资料根完整 93 项测试通过。待所有者明确正式发布 0.18.1 后重建连接并核对李爱琳rene最新 939 字 official 原文；不得读取 Cookie、密码或改用 API Key |
