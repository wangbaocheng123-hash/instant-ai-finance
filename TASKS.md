...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布并启用 ChatGPT 直连新加坡博主 MCP | 0.18.1 已正式上线：GitHub `main` 基线 `62aaaae`，正式健康接口、Service Worker 与 MCP `initialize` 均返回 0.18.1；OAuth 元数据、两个只读工具、GET 405、未授权 401 与标准挑战通过。DCR 现为每个 connector instance 独立客户端并自动迁移旧库，发布前完整 93 项测试通过。剩余工作是删除旧插件外壳、重建连接并核对李爱琳rene最新 939 字 official 原文；不得读取 Cookie、密码或改用 API Key |
