...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布并启用 ChatGPT 直连新加坡博主 MCP | 0.18.2 已正式上线：生产提交 `8bd242f`，服务器 93 项测试、公网 OAuth/MCP 与真实授权页结构通过；授权页自动带入当前单主人账号、只需密码，并保存 30 天网页会话，MCP token 与 Cookie 继续隔离。剩余工作是删除旧连接外壳、重建连接并核对李爱琳rene最新 939 字 official 原文；不得读取 Cookie、密码或改用 API Key |
