...
| P3-CLOUD-04 | BLOCKED | 把 0.12.1 界面精简版正式发布到阿里云 | GitHub `main` 已到 `3f94db1`；正式站点仍为 0.12.0。当前 Codex 环境无 Alibaba Cloud Client 控制通道，生产仓库为 root 所有且 `sudo` 需要交互密码；按永久规则等待从客户端服务器连接执行既定更新脚本，不绕过权限或回退网页/外部 SSH |
