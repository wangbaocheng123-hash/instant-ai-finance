...
| P3-BLOGGER-02 | NOT_STARTED | 正式发布 0.18.0 并启用 GPT 博主云端只读通道 | 等待所有者明确“正式发布即时 AI 0.18.0”；随后只用既有受限发布器部署，通过专用脚本生成独立凭据并以服务器 root 0600、本机 DPAPI 保存，重启本地 MCP 后实测李爱琳rene最新 `cloud-video:` 正式原文；不得输出凭据或复制业务数据库 |
