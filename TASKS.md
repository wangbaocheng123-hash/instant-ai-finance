...
| P3-BLOGGER-02 | NOT_STARTED | 正式发布 0.18.0 并启用 ChatGPT 直连新加坡博主 MCP | 等待所有者明确“正式发布即时 AI 0.18.0”；随后只用既有受限发布器部署 `/mcp` 与 OAuth，在 ChatGPT 开发者模式新增“博主智能体（云端）”连接，以现有主人账号授权，并实测手机/电脑搜索与读取李爱琳rene最新 `cloud-video:` 正式原文；不得输出凭据、复制业务数据库或依赖本地 8775 |
