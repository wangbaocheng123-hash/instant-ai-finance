...
| P2-MODEL-10 | DONE | 为 GPT/MCP 增加模型先生只读内容调用 | ADR-0032；0.19.0 候选在原云端连接保留两个博主工具并新增作品搜索、完整作品文字/已有投资解读、投资思路查询三个工具。投影只读 Git 外快照和详情，排除评论、媒体、路径、凭据与管理字段，不调用 sidecar、ASR、豆包、AI 或写入；针对性 15 项、完整 Python 101 项、Vite 构建和 npm 零漏洞审计通过 |
| P3-MODEL-01 | IN_PROGRESS | 正式发布并验收模型先生云端 MCP | 0.19.0 候选已完成并保持既有 OAuth 连接兼容；待所有者明确“正式发布”后由受限发布器上线，在 ChatGPT 点击 Refresh 验证共五个只读工具，并对模型先生最新作品执行真实 search/get，不触发识别、AI 或资料写入 |
