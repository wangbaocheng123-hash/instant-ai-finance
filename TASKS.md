...
| P3-CLOUD-04 | DONE | 把 0.12.1 界面精简版正式发布到阿里云 | 用户从 Alibaba Cloud Client 的 root 终端执行既定更新脚本，生产仓库快进至 `5f2e851`，服务器 21 项测试通过并重启服务；正式/应急/回环健康均为 0.12.1，SW 为 0.12.1，线上搜索和统计标记不存在，20/20 来源健康、时变罗盘完整性 `ok` |
