...
| P3-BLOGGER-02 | IN_PROGRESS | 正式发布并启用 ChatGPT 直连新加坡博主 MCP | 0.18.1 已正式上线并完成 DCR 隔离修复；0.18.2 候选进一步让 OAuth 页从当前单主人配置自动带入账号、醒目提示错误并保存授权登录后的 30 天网页会话，MCP token 与 Cookie 继续隔离。针对性 7 项通过，待明确正式发布后删除旧连接外壳、重建连接并核对李爱琳rene最新 939 字 official 原文；不得读取 Cookie、密码或改用 API Key |
