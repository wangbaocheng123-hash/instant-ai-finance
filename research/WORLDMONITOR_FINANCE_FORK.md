...
- 产品分叉提交：`658dce646c59aedd02efd265e655085e36155293`
- 许可证：`AGPL-3.0-only`
...
- `src/instant-ai/InstantFinanceApp.ts`：顶部单一“即时热点”滚动栏、12 个财经频道、来源和详情交互；滚动栏将按发布时间排序的最新消息与按近 72 小时多来源数、重要度和新鲜度排序的热点交错去重；频道采用单页状态直切，桌面导航与手机底栏共用选中状态，不再用平滑滚动定位面板；只显示自动实时采集状态，不提供手动采集按钮。0.12.1 起删除统计概览、客户端搜索和依赖搜索的区域快捷入口。
...
- 即时热点检查：原“即时”和“全球热点”已合并为一条全宽滚动栏；最新消息与多来源/高重要度热点各取候选并交错去重，首轮 16 条唯一事件复制为 32 个按钮实现无缝滚动，点击沿用同一证据详情。
...
- 0.7.2 单栏检查：桌面与 390×844 手机端均只有 1 条“即时热点”，手机栏高 36px；桌面首轮 16 条全部去重，同时出现最新消息、`2源` 多来源热点及 75/84 分高重要度事件；生产构建与 14 项单元测试通过。
- 0.12.1 界面精简检查：TypeScript/Vite 生产构建不再包含统计概览、搜索框、搜索结果或区域搜索快捷入口，新闻面板扩展至腾出的视口空间；npm 零高危审计与 21 项 Python 测试通过。
