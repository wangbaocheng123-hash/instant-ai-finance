...
- `src/instant-ai/InstantFinanceApp.ts`：顶部单一“即时热点”滚动栏、12 个财经频道、搜索、来源和详情交互；滚动栏将按发布时间排序的最新消息与按近 72 小时多来源数、重要度和新鲜度排序的热点交错去重；频道采用单页状态直切，桌面导航与手机底栏共用选中状态，不再用平滑滚动定位面板；只显示自动实时采集状态，不提供手动采集按钮。
