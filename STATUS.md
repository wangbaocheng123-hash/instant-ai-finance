...
- 北京采集套件迁移当前有一个外部控制面闸门：新加坡 ECS 尚未绑定最小 RAM 实例角色，北京服务器也尚未安装 `beijingcodex` 两个固定包装器；因此本轮无法真实盘点原模型下载器源码/运行单元，也不能开始源码导入或统一发布器迁移。仓库、CLI 和失败关闭的安全模板均已就绪，现有北京生产保持不变。
...
`完成 ADR-0045 阶段 A 并只读盘点北京模型下载器`：由可信阿里云控制端把仓库内最小策略绑定到当前新加坡 ECS、在北京安装两个固定包装器；随后本云端 Codex 通过 `EcsRamRole` 与 Cloud Assistant 执行 `inspect`，只取得真实 service/源码 Git 元数据，不读业务正文或凭据。盘点通过后再导入原模型下载器源码并实施 ADR-0044。
