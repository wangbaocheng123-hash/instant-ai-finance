...
- 最后更新：2026-09-05
- 即时 AI 新标识与紧凑页头候选（未发布）：使用所有者提供的方形蓝色“即时 AI”图作为页面、登录页、PWA 192/512 与 iPhone 180 图标；手机端把标识、名称、汉化、来源、退出和实时采集合并为单行，页头压缩至 44px 且 390px 无横向溢出。151 项 Python 回归、7 项前端契约、TypeScript/Vite 构建、生产依赖零漏洞审计，以及 390/1280px 隔离浏览器截图回归通过；正式站仍为旧图标和旧页头，未发布、未改业务资料、未触发采集或 AI。
...
- 当前阶段：`P2/P3 — 即时 AI 0.19.2 正式生产；模型先生全部作品已改为滚动按需分页`
...
- 完成 `P2-UI-08` 开发候选。旧“即”字标识已从前端源码、favicon、PWA manifest 和 iPhone touch icon 引用中替换；新 PNG 为所有者图片的等比例机械缩放，不生成或改写画面。Service Worker 使用独立品牌缓存键，正式发布后不会继续命中旧 SVG 外壳。手机页头实测标识与四项工具同处一行，高度 44px、无横向滚动；生产静态目录和线上服务尚未更新。
