...
- 最后更新：2026-09-09
- 即时 AI 0.21.4 模型先生送达自动处理候选已完成，尚未正式发布；生产仍为 0.21.3。根因已确认：北京视频及评论成功送达并投影，但旧模型先生队列默认关闭，关闭时 `complete` 回调直接丢弃任务，之后没有漏回调补偿，所以并非豆包识别失败，而是豆包从未收到任务。
...
- 0.21.4 开发验收：189 项完整 Python 回归、8 项前端源码/品牌契约、Python 编译、TypeScript/Vite 生产构建和 npm 高危阈值审计通过，真实付费调用 0 次。隔离 Playwright 流程已更新并完成编译，但当前云端 Chromium 仍缺系统 `libatk-1.0.so.0`，未安装系统依赖，也未伪报浏览器运行通过。下一项唯一建议任务：所有者明确说“正式发布”后，经受限发布器上线 0.21.4，并核对最近 48 小时内当前漏项进入队列后完成原文与关键词。
...
`手机抽查 0.21.3 历史作者互动`：完全关闭并重新打开即时 AI，进入截图对应模型先生作品的“作者互动”；3 组互动应各自先显示粉丝原问题、再显示模型先生红色作者回复，评论总数仍为 515。若保留评论包缺失或版本不匹配，记录会安全跳过而不猜测关系。
