...
- 最后更新：2026-09-10
- 最新检查点：ADR-0046 受限 SSH 控制入口已在独立开发分支实现，尚未服务器安装或现场连通验收。新增固定命令分发、仅元数据状态回执、默认检查且遇已有冲突停止的安装器，以及新加坡侧检查/一次触发/结果验证客户端。现有业务源码、生产分支和 HTTPS 媒体通道未改。
- 现场目标更正：可信 Alibaba Cloud Client 显示北京是 SWAS 轻量应用服务器 `e905b859a6c24c529ef5c2ea21658462`，北京 AccountId 仍未核实；不可照用旧 ECS RunCommand/RAM 模板。客户端仍显示已登录，但输入操作返回 `coordinate input geometry is unavailable`，键盘焦点也未可靠进入服务器连接。因此不能宣称已打开 root 终端或完成安装。
- 当前已完成隔离控制测试，最终测试结果见本次 CHANGELOG；尚未完成 SSH 主机公钥可信核对、专用密钥在新加坡生成、北京授权安装、跨区网络与越权现场验证、电脑关机后的手机验收。不要重复把旧“CLI 已装/模板已备好”写成“权限已通”。
...
- 当前新加坡主机已安装并校验官方 Alibaba Cloud CLI 3.4.11，建立无 AccessKey 的 `EcsRamRole` 配置轮廓；仓库新增精确到北京实例与低权限 `beijingcodex` 的 RAM 策略模板、北京只读盘点器、现有一次性发布固定触发器、root 安装器和只允许 `inspect/publish` 的 Cloud Assistant 客户端。当前 ECS 的 IMDS 仍返回无 RAM 角色，因此尚不能真实调用北京实例；需要可信阿里云控制面完成一次角色绑定与北京包装器安装。
...
- 当前控制入口闸门改为：取得可信客户端内可操作的北京 root 连接，核实真实账号/主机和新加坡出口，按 ADR-0046 安装并现场验证受限 SSH。当前工具无法可靠点击客户端；未读取保存的凭据，未切换浏览器绕过，也未安装服务器文件。不是继续扩大 RAM 角色权限。
...
- ADR-0045 架构、仓库内过渡底座和用户级官方 CLI 安装已获所有者本轮明确批准，不需要重复确认。RAM 角色绑定和北京 root 包装器安装属于尚未具备权限的外部控制面动作；本轮未擅自执行或将其误报为完成。
...
`完成 ADR-0046 可信首次安装与新加坡只读验收`：先在现有客户端 root 连接核实北京身份和 SSH 主机公钥，在新加坡生成专用密钥并验证出口；从已审查的精确提交运行安装器，随后从新加坡执行 inspect/status 和拒绝越权验证。本步骤不推进 beijing-production、不发布业务、不启用 timer。只有现场接通后才开始真实模型下载器源码盘点。
