...
- 阶段状态：`LOCAL_0_9_1_VERIFIED / GITHUB_MAIN_SYNCED / GRANDPAAMU_0_9_1_LIVE`
- 产品名称：`即时 AI`
...
- 本机正式端和阿里云端均已发布并验证 `0.9.1`；本机 Python 服务监听 `127.0.0.1:18765`，桌面快捷方式和手机预览快捷方式均存在。
...
- 云端正式域名和应急域名的 `/api/health` 均健康返回 `0.9.1`；manifest 为 browser 模式，Service Worker 为 0.9.1，页面已包含“浏览器翻译原文”和“中文摘要（备用）”，不再包含旧正文抓取提示。更新后的第 54 次自动采集于 2026-08-28 18:55:01 成功：20 个来源、抓取 681 条、新增 56 条、错误 0；自动模式和 300 秒周期正常。翻译目标为 `zh-CN`；AI 证据契约为 `evidence-v1`，真实模型尚未配置，采集、证据、规则评分和阅读不受影响。
...
- 本机、GitHub、正式域名和应急域名均已运行 0.9.1，没有版本发布阻塞。服务器仓库快进到 `8de1605` 后保持干净，`instant-ai.service` 与 Caddy active；原罗盘 `127.0.0.1:8080` 健康状态仍为 200。
...
- 本轮 GitHub、阿里云 0.9.1 和自有域名验收均已完成，不需要新增账户、域名或服务器授权。
