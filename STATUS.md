...
- 0.18.4 真实新连接已经使用独有 callback-ID，DCR、授权页、主人凭据和 302 回调均成功；等待后仍没有任何 `/oauth/token` 请求。结合 0.18.3 稳定回调同样表现，已排除账号名、密码、按钮、DCR 客户端复用和回调模式，服务端完整流也由 95 项回归覆盖。断点现收敛为浏览器中的 ChatGPT/Codex 回调会话；下一步需要点击后包含地址栏的页面状态，判断回调是否停留在 grandpaamu.com、chatgpt.com 登录/错误页或被浏览器拦截，不再修改服务器认证。
