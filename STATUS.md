...
- 最后更新：2026-09-15 22:01（北京时间）
- 最新检查点：`MODEL_MR_COMPLETE_MCP_0_22_1_CANDIDATE`。公网 0.22.0 的 MCP 连接正常，但 `tools/list` 只有六个只读工具：作品工具按既有 ADR 主动排除评论正文，作者回复工具又只开放来源确认的模型先生发言和原提问，因此 GPT 看不到完整粉丝评论楼、全部互动、十类关键词、评股和作品关联思路。根因是服务端工具契约，不是提示词、GPT 账号或评论采集连接故障。
- 0.22.1 候选保留全部旧工具和单主人 OAuth，新增第七个 `get_model_mr_comments`：按作品分页读取全部已投影评论，区分模型先生本人和粉丝，保留同楼关系、时间、点赞/回复计数及作者赞过标记，并支持全部、本人互动、仅本人、仅粉丝和作者赞过五种视图。作品搜索增加 `offset/next_offset/has_more`，可翻遍当前模型先生库并检索标题、关键词、原文、解读、评股和评论；原作品工具补齐标题来源、十类 AI 关键词、原文、已有解读、评股、关联投资思路和评论统计。
...
- 候选验收：200 项即时 AI Python 回归、14 项前端源码/共享评论逻辑契约、Python 编译、TypeScript/Vite 0.22.1 生产构建、八项静态资源逐字节一致及 npm 高危阈值审计全部通过；真实模型调用 0 次、生产资料读写 0 次。当前尚未收到本轮“正式发布”指令，公网仍是 0.22.0 和六个工具；下一项唯一发布任务是获得明确指令后走既有受限发布器，并验收公网 0.22.1、七工具列表及主人授权下的一次真实只读分页调用。
