...
- `news.amuyeye.com` 当前尚未创建解析；自有域名绑定只允许通过 `Alibaba Cloud Client` 完成，需先确认该客户端是否提供 AliDNS 记录管理能力。若不支持，必须报告限制，不得回退浏览器或提取客户端凭据。
...
`P3-DOMAIN-01`：仅通过桌面 `Alibaba Cloud Client` 核实并尝试新增 `news.amuyeye.com`；若客户端无 AliDNS 功能，停止并报告，不使用浏览器替代。
