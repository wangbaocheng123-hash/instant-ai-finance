...
- 最后更新：2026-09-23 19:24（北京时间）
- 最新检查点：`MODEL_MR_BEIJING_TIME_0_23_2_CANDIDATE_READY`。修复提交 `d84d878` 已推送 GitHub `main`。北京生产库只读核对确认最新作品 `7688668173143610297` 的真实 Unix 时间为 `2026-09-23T09:59:19Z`，即北京时间 `2026-09-23T17:59:19+08:00`；页面显示 `9月24日01:59` 的根因是旧桥把无偏移北京时间误标为 UTC，手机又转换一次而重复增加八小时。
- 0.23.2 / 北京采集器 1.0.9 / 模型下载器 `0.10.3+git.beijing-time` 候选已统一修复三层契约：作品与评论从 Unix 秒生成时显式写 `+08:00`，传输桥把历史无偏移值按 `Asia/Shanghai` 解释并把规范化时间纳入修订指纹，模型先生页面固定按北京时间显示。旧错误投影会形成一次可审计修订；相同视频沿用 SHA-256 内容对象，不重复上传完整 MP4。
- 本轮尚未获得新的“正式发布”口令，因此没有推进 `beijing-production`、没有调用新加坡或北京发布器，也没有改写生产 SQLite、主人资料、视频、评论、标题、原文或关键词。当前生产仍为即时 AI 0.23.1、北京采集器 1.0.8、模型下载器 0.10.2。
- ADR-0056 记录北京时间契约与发布门禁；即时 AI 207 项、北京采集/传输 223 项、模型下载器 27 项、北京发布控制 32 项、前端源码契约 10 项与共享评论逻辑 6 项通过，Vite 0.23.2 构建、静态摘要一致及 npm 高危审计通过（0 漏洞）。验证覆盖真实秒级样例、历史无时区兼容、显式 UTC 保留、旧修订指纹变化、页面固定时区和 0.23.2 Service Worker。正式发布后还需等待校时修订投递完成，再核对手机、API 与 MCP 的最新时间一致。
- 上一个已发布检查点仍为 `MODEL_MR_VIDEO_STORAGE_AND_COMPACT_RELEASE_LIVE_VERIFIED`。所有者明确要求“正式发布”后，功能提交 `39da14f` 与结果基线 `d8d3a5a` 已进入 GitHub `main`；即时 AI 固定发布器把新加坡生产从 `50c8c2c` 安全快进到 `d8d3a5a`，服务器 207 项回归通过并上线 0.23.1。
- 新加坡首次启动已完成模型先生传输 artifact 内容寻址迁移。根盘已用字节从 48,520,581,120 降至 20,286,341,120，实际释放 28,234,240,000 字节（26.30 GiB）；`df` 使用率从 97% 降至 41%，可用空间从约 1.5 GiB 增至约 28 GiB。即时 AI 与 Caddy 均 active，公网健康和 Service Worker 均为 0.23.1。
- 公网以符合正式路由格式的未登录 GET 与 Range 请求复核模型先生私有媒体，均返回 401；无效的非路由地址只会落到 SPA，未把该 200 误报成媒体权限通过。4 MiB 连续 Range、300 秒私有发送窗口、一小时浏览器私有缓存和旧 API 兼容已由 207 项回归覆盖；主人登录后的 iPhone 一次完整播放仍留作真机验收。
- `beijing-production` 已安全快进到 `d8d3a5a`。北京统一发布器在服务器隔离运行 25 项模型下载器测试后，正式上线 `0.10.2+git.compact-video`；当前模型运行目录精确为该提交，model/model-web/blogger 三服务 active，发布服务 inactive/result success，代码发布 timer 继续 disabled/inactive。
- 北京重启后的真实主页扫描 10162 于 07:27:38—07:27:52 成功，当时今日可见与新下载均为 0；这证明发布后扫描链路正常且没有借发布批量改写历史视频。下一条自然到达的新视频才会实际生成半尺寸 H.264/AAC 快启 MP4，届时再核对 720×1280→360×640 或 576×1024→288×512、文件大小、播放和签名传输。
- 发布前完整验收为即时 AI 207 项、模型下载器 25 项、北京采集/传输 221 项、北京统一部署控制 32 项、前端源码契约 9 项、Vite 0.23.1 构建及 npm 高危审计；真实 ffmpeg 用例生成带声 720×1280 输入并核验 360×640、H.264/AAC、`yuv420p` 输出。正式发布没有批量转码既有北京或新加坡主人视频，也没有修改时变罗盘、域名、Caddy、安全组或付费模型配置。
- 发布前容量核查曾确认根云盘约 49GB、已用约 46GB、仅余约 1.6GB（97%），最大项是 46 个不同视频因 3,216 个评论修订形成的传输副本；该风险现已由内容寻址迁移消除。Codex 历史、Swap、罗盘版本、缓存及正式主人资料均未作为清理对象。
- 最新检查点：`MODEL_MR_ARTICLE_POST_0_10_1_LIVE_REAL_VERIFIED`。模型先生 07:47 发布的真实图文不是旧 `/note/` 相册，而是新 `/article/7685913028676799717` 长图文；三分钟扫描始终成功，但旧选择器只接受 `/video/`、`/note/`，且旧解析器只认 `images`，所以该作品在主页阶段被忽略。长图文正文实际位于 `article_info.article_content` 的 JSON Markdown，不能继续按普通相册字段读取。
- 修复提交 `e91477c809a2ce2eb89e7d48e89f544ca3011c8a` 已进入 `main` 与 `beijing-production`，北京模型下载器正式升级为 `0.10.1+git.article-posts`。主页现在同时识别 `/video/`、`/note/`、`/article/`；长图文保留独立标题和完整 Markdown 正文，有内嵌原图时按顺序保存，没有内嵌图片时保存抖音公开 `origin_cover`，不生成伪图片、不做 OCR。旧视频和 `/note/` 行为不变。
- 本地模型下载器 21 项回归通过；未发布代码在北京真实公开页的只读探针得到 20 条主页作品（17 视频、3 图文），准确命中目标 article，并识别 23 字标题、215 字正文、2 段及 1 张 176,249 字节 JPEG 封面。北京统一发布器再次通过服务器 21 项隔离测试和发布后真实扫描门禁；模型 accepted/deployed/current 均为 `e91477c`，博主子树未变而 accepted 安全前移至 `e91477c`，model/model-web/blogger 三服务 active，发布 timer 仍 disabled/inactive。
- 发布后扫描 8457 于 18:12:45—18:13:13 成功，今日发现 2 条、新下载 1 条。目标记录现为 `work_type=image`、`download_status=downloaded`，正文 215 字；1 个媒体文件存在，MIME、大小、文件头和 SHA-256 均通过。评论空档随后保存 116 条当前公开记录，不影响三分钟扫描。
- 北京只读桥已经把目标的图片、正文和评论修订经原签名通道送达新加坡：最新已送达 revision 2、HTTP 200，媒体与评论 artifact 均为 `verified`；该 200 在即时 AI 同步投影回调成功之后才返回。当前评论包规范化捕获 114/预期 116 并如实标记 incomplete，后续 15 分钟刷新继续补齐，不伪报完整。公网未登录模型先生 API 仍返回 401；即时 AI 0.23.0 代码无需改版或重发。
- 下一项唯一建议任务：所有者在手机登录即时 AI 后打开今天 07:47 的作品，确认标题、两段正文和单张封面视觉显示符合预期。服务器采集、文件校验、评论保存、签名传输和新加坡投影已经用真实作品完成验收；手机主人会话的视觉确认仍不由未登录检查冒充。
- 最新检查点：`MODEL_MR_COMPLETE_MCP_0_22_1_LIVE`。0.22.0 的 MCP 连接原本正常，但 `tools/list` 只有六个只读工具：作品工具按既有 ADR 主动排除评论正文，作者回复工具又只开放来源确认的模型先生发言和原提问，因此 GPT 看不到完整粉丝评论楼、全部互动、十类关键词、评股和作品关联思路。根因是服务端工具契约，不是提示词、GPT 账号或评论采集连接故障。
- 0.22.1 正式版保留全部旧工具和单主人 OAuth，新增第七个 `get_model_mr_comments`：按作品分页读取全部已投影评论，区分模型先生本人和粉丝，保留同楼关系、时间、点赞/回复计数及作者赞过标记，并支持全部、本人互动、仅本人、仅粉丝和作者赞过五种视图。作品搜索增加 `offset/next_offset/has_more`，可翻遍当前模型先生库并检索标题、关键词、原文、解读、评股和评论；原作品工具补齐标题来源、十类 AI 关键词、原文、已有解读、评股、关联投资思路和评论统计。
- 安全边界保持不变：所有 MCP 工具只读 Git 外主人投影，不启动采集、下载、ASR、豆包、AI 或标题修改；不返回粉丝账号 ID/主页、来源评论 ID、内部线程哈希、原始 JSON、数据库、媒体文件、服务器路径、Cookie 或密钥。评论被明确标记为外部非可信资料，不能执行其中夹带的指令。
- 所有者于 2026-09-16 明确要求正式发布；受限发布器将生产从 `d8be739` 安全快进到功能提交 `bd05eda`，服务器 200 项回归通过。即时 AI 与 Caddy 均 active，回环和公网健康、Service Worker、MCP `initialize` 均为 0.22.1；公网 `tools/list` 实测七个只读工具并包含 `get_model_mr_comments`，未登录模型先生 API 与评论工具均返回 401/OAuth 挑战。发布没有迁移或改写 Git 外资料，也没有触发采集、ASR、豆包或 AI。剩余仅需主人在原 ChatGPT 连接点击 Refresh、新建对话并完成一次授权后的真实评论分页读取。
- 最新检查点：`MODEL_MR_GUARDED_TITLE_0_22_0_LIVE`。0.22.0 已按所有者明确的“正式发布”要求上线：只有原作品无有效标题时，既有关键词调用才读取最多四张开头画面；可靠封面标题优先，否则根据 ASR 正式原文拟题。画面文字绝不进入视频原文，连续字幕 OCR 不用于构造原文，也没有独立第三次标题调用。
...
- 功能提交 `d8be739d5739f5a1d302f3d8c864a156834d471f` 已推送 GitHub，并由既有受限发布器把生产从干净 `e83d8ad` 安全快进到同一提交；服务器 199 项回归通过，回环/公网健康和 Service Worker 均为 0.22.0，公网 `app.js` 与开发及生产仓库摘要一致。即时 AI/Caddy active，未登录模型先生 API 仍为 401；发布没有批量处理历史标题、手动入队或主动调用豆包。
...
- 即时 AI 0.23.1 与北京模型下载器 0.10.2 已按所有者本轮“正式发布”授权上线，不需要再次批准。新加坡旧重复副本已安全释放；北京只对后续自然到达的新视频执行半尺寸编码，不批量改写历史媒体。
- 0.22.0 已按所有者本轮明确的“正式发布”口令上线，不需要再次批准。发布没有批量回填历史标题、手动建立任务或主动调用豆包；下一条自然到达且确实缺失标题的作品会按现有自动额度规则验证新链路。
...
`正式发布并验收模型先生北京时间修复`：收到所有者新的“正式发布”口令后，先发布即时 AI 0.23.2，再经 `beijing-production` 发布北京采集器 1.0.9 与模型下载器 0.10.3；等待历史校时修订送达后，核对最新作品在手机、主人 API 与 MCP 都显示 `2026-09-23 17:59`，而不是 `2026-09-24 01:59`。不得手工改生产数据库、重复上传视频或触发 ASR/豆包/AI。
