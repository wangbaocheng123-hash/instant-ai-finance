...
- 最后更新：2026-09-20 07:22（北京时间）
- 最新检查点：`MODEL_MR_CONTENT_DEDUP_AND_COMPACT_VIDEO_CANDIDATE_VERIFIED`。46 个不同的模型先生视频因评论变化形成 3,216 个已送达修订，旧接收布局把同一 MP4 按 transfer 重存，传输媒体累计 26.63 GiB、不同媒体实际仅 0.38 GiB；约 26.24 GiB 是重复视频副本。ADR-0055 已完成内容寻址修复：新修订复用已验证 SHA 并从 `missing_artifacts` 排除相同视频，评论更新以后只传评论包。
- 启动压缩迁移保留全部 transfer、revision、处理队列与审计行，先建立并完整校验 `artifacts/objects` 内容对象，再用 SQLite 事务改指向，最后只释放零引用且严格匹配旧自动生成目录的普通文件；中断后可继续，异常文件保留。按现场台账，正式启动预计释放约 26.2 GiB，不删除正式模型先生视频、标题、原文、关键词、评论、解读或思路。
- 北京模型下载器 `0.10.2+git.compact-video` 候选把后续新视频统一转为 H.264/AAC、`yuv420p`、64k 音频和 faststart，通常长宽各减半：现场主要 720×1280 将变为 360×640，576×1024 将变为 288×512；宽度最低 240 像素。既有北京和新加坡正式视频不批量覆盖。
- 当前验收：即时 AI 207 项、模型下载器 25 项、北京采集/传输 221 项、北京统一部署控制 32 项完整回归，以及 9 项前端源码契约、Vite 0.23.1 构建、npm 高危审计均通过；新增真实 ffmpeg 用例已生成带声 720×1280 输入并核验输出为 360×640、H.264/AAC、`yuv420p`。生产私有资料尚未改写，重复副本尚未清理；仍须按既有门禁正式发布后，以启动压缩摘要、`df` 和完整性抽查确认实际释放量。
- 最新检查点：`MODEL_MR_MOBILE_VIDEO_RESUME_0_23_1_CANDIDATE_VERIFIED`。模型先生手机本地视频中途卡住已定位为传输链路问题：服务统一的 30 秒 socket timeout 也约束写入，iPhone 暂停读取或网络变慢时会截断响应；`bytes=N-` 旧实现一次发送到 EOF，生产 Caddy 又给全部 `/api/*` 追加 `no-store`，所以第二次、第三次点击只能靠重新读取逐步补齐。
- ADR-0054 / 0.23.1 候选已把开放 Range 限为每段 4 MiB，只在私有媒体正文发送期间把超时从 30 秒提升到 300 秒并立即恢复。页面返回显式主人认证的 `/media/model-mr/` 地址，保留旧 API 兼容；媒体允许一小时浏览器私有缓存，Service Worker 不缓存视频或原图。播放器显示等待和自动续传状态，但不强制重载破坏 iPhone 用户手势。
- 204 项完整 Python 回归、9 项前端源码契约、TypeScript/Vite 生产构建、静态产物一致和 npm 高危审计通过。测试覆盖未登录视频/图片 401、精确与开放 Range、4 MiB 边界、HEAD、旧路由及 socket 超时恢复；没有触发生产资料读写、采集、ASR、豆包或 AI。
- 即时 AI 0.23.1、接收端去重迁移和北京模型下载器 0.10.2 已随候选提交 `39da14f` 推送 GitHub `main`，但尚未正式发布，也尚未推进 `beijing-production`。必须等待所有者另行明确要求“正式发布”，之后才使用既有受限发布器，并以公网连续 Range、同一视频一次完整播放、启动去重摘要、磁盘实际释放量和下一条自然到达视频的半尺寸编码作最终验收。
- 同轮只读容量核查：根云盘约 49GB，已用约 46GB，仅余约 1.6GB，使用率 97%；inode 仅 14%，所以是硬盘字节容量而不是运行内存或文件数耗尽。可见大项有 `~/.codex` 约 5.1GB（其中旧/现 Codex 程序包约 3.8GB、会话约 1.17GB）、`~/.cache` 约 1.1GB（其中 Playwright 浏览器约 919MB）、`/opt/time-compass` 约 1.8GB、`/tmp` 约 840MB；`/var/lib/instant-ai` 为业务账号私有目录，本轮没有越权读取、更没有删除视频或业务资料。容量风险会影响后续新视频、备份和临时发布，但不是本次现有视频中途卡住的直接原因。
...
- 即时 AI 0.23.1 现在同时包含手机续播与传输媒体全局去重；北京 0.10.2 包含新视频半尺寸轻量化。代码和测试不等于生产清理，仍须所有者明确要求“正式发布”后，才可分别推进即时 AI 生产与 `beijing-production`，并释放旧重复副本。
...
`正式发布存储治理并核验实际释放量`：先上线即时 AI 0.23.1，让启动迁移合并旧传输视频并确认磁盘、账本、正式视频和评论完整；再发布北京模型下载器 0.10.2，使下一条新视频按半尺寸保存。两端均不得批量转码既有正式视频或触发历史 ASR/豆包任务。
