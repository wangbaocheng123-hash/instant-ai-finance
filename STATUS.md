...
- 最后更新：2026-09-07
- 即时 AI 0.21.1 博主自动处理修复已完成开发候选，尚未正式发布。根因是 0.21.0 的自动队列只依赖传输完成瞬间回调，开启前作品和短暂漏回调不会入队；手机处理区又不自动轮询，后台状态变化后页面仍可能看起来“没反应”。
- 候选现持久保存每次手动开启的时间边界：complete 回调继续即时入队，后台每三秒补偿核对开启后完成且 MP4 已验证的普通博主视频，短暂资料库不可读不推进游标，重复回调和扫描按媒体摘要去重。关闭期间或开启前历史不会自动批量扣费。
...
- 0.21.1 候选已通过 168 项完整 Python 回归、7 项前端源码/品牌契约、6 项共享评论逻辑、TypeScript/Vite 构建、JavaScript/Python 语法和 npm 高危阈值审计。隔离浏览器流程脚本已新增并通过语法检查；当前云端 Chromium 因系统缺少 `libatk` 未能启动，未擅自安装系统依赖。测试没有读取生产资料、开启生产队列或调用真实豆包。
...
- 当前阶段：`P2/P3 — 即时 AI 0.21.1 博主自动处理修复候选待发布；0.21.0 与北京采集器 1.0.8 正式生产`
...
- 0.21.1 代码与自动化验收无功能阻塞，但尚无本轮正式发布授权，生产仍为 0.21.0。云端开发机浏览器缺少 `libatk`，因此没有执行截图脚本；该系统依赖未擅自安装，不影响 Python/TypeScript/前端契约与生产构建结果。
...
- 0.21.1 候选正式上线仍需所有者新的明确发布口令。代码发布不会自动处理开启前历史；若要验证“贵族之路”当前旧漏项，发布后由主人在具体作品点一次“一键补做”并确认费用，或等待下一条新视频验证全自动链路。
...
`正式发布并真机验证 0.21.1`：所有者明确要求正式发布后，先通过既有受限发布器上线且不触发模型调用；随后在“贵族之路”选一条开启前漏项点一次“一键补做”，或用下一条新传视频观察自动入队，并核对页面依次显示视频原文、AI关键词和完成状态。
