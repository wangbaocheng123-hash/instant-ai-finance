...
- 阶段状态：`LOCAL_0_10_0_WATCH_PREVIEW_VERIFIED / GRANDPAAMU_0_9_2_LIVE`
- 产品名称：`即时 AI`
...
- 0.10.0 开发候选已完成“重点事件关注”第一阶段：只读汇总时变罗盘首页 14 个和紫金 31 个未来节点，在即时 AI 独立 SQLite 中保存并用五分钟当前消息窗口做匹配；手机版入口位于 AI 右侧。暂不包含 Codex 分析、罗盘回填或系统通知，见 ADR-0016。
- R0 已结束；六个正式上游仓库和许可证证据继续作为历史基线。
- 本机正式端和阿里云端均已发布并验证 `0.9.2`；本机 Python 服务监听 `127.0.0.1:18765`，桌面快捷方式和手机预览快捷方式均存在。
...
- 0.10.0 开发候选的 SQLite schema 为 7，新增罗盘重点事件、匹配和同步状态；正式端 0.9.2 在发布前仍为 schema 6。`reader_translations` 从 0.9.1 起只缓存用户实际请求的来源摘要译文，并在消息到期时级联删除。`summary-only-v1` 指纹使旧正文摘录缓存不再命中。首次正式清理把约 3,758 条累积消息缩减到约 2,317 条当前窗口消息，把 173,806 份重复证据缩减到 2,328 份，`VACUUM` 后数据库由约 268 MiB 缩减到约 8 MiB，`integrity_check=ok`。
...
- 云端正式域名和应急域名的 `/api/health` 均健康返回 `0.9.2`；Service Worker 为 0.9.2，公网 `app.js` 已确认包含 iPhone Chrome 直达 scheme 和“普通浏览器备用打开”。更新后的第 60 次自动采集于 2026-08-28 19:21:37 成功：20 个来源、抓取 679 条、新增 59 条、错误 0；自动模式和 300 秒周期正常。翻译目标为 `zh-CN`；AI 证据契约为 `evidence-v1`，真实模型尚未配置，采集、证据、规则评分和阅读不受影响。
...
- 重点事件功能已通过开发区接口、19 项单元测试、生产构建和桌面/手机预览，但尚未正式发布；生产站继续运行 0.9.2，不会提前出现新入口。
- 本机、GitHub、正式域名和应急域名均已运行 0.9.2，没有版本发布阻塞。服务器仓库更新后保持干净，`instant-ai.service` 与 Caddy active；原罗盘 `127.0.0.1:8080` 健康状态仍为 200。
...
- 0.10.0 重点事件功能正式发布仍需用户明确回复“正式发布”。
- 本轮 GitHub、阿里云 0.9.2、正式域名和应急域名验收均已完成，不需要新增账户、域名或服务器授权。
