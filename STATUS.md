...
- 最后更新：2026-09-16 14:08（北京时间）
- 最新检查点：`MODEL_MR_IMAGE_POSTS_0_23_0_LOCAL_CANDIDATE`。最新版根因已定位并完成本地候选：模型先生主页扫描原先只认 `/video/`，解析、SQLite、北京桥、新加坡投影和手机页面也都固定为单个 MP4，因此 `/note/` 图文在入口即被漏掉。生产只读核对仍为北京 `0.9.2+git.video-priority-3m`、即时 AI 0.22.1；模型库现有 37 条记录中没有 `/note/`，所以本轮没有伪造真实图文验收。
- ADR-0053 / 北京 `0.10.0+git.image-posts` 候选现可识别视频和图文：保存公开完整正文以及按发布顺序排列的全部 JPEG/PNG/WebP 原图，schema 5 用 `work_media` 保存每张图片的大小和 SHA-256；管理页可预览、逐张下载、修复和删除。失败或损坏不会把半套图片标记完成，旧视频数据与三分钟硬优先调度保持不变。
- 既有签名传输现在按单图 `image` / 多图 `gallery` 发送全部原图和正文；即时 AI 0.23.0 候选把它们投影到 Git 外模型先生资料库，通过主人认证的私有图片接口和手机原图库展示，MCP 只返回作品类型、图片数和公开正文。图文不进入视频 ASR/自动处理队列，不产生豆包费用；图片像素中的文字本轮不做 OCR，也不会混入公开正文。
- 本地验收全部通过：模型下载器 18 项、北京采集/传输 221 项、北京统一部署控制 32 项、即时 AI 202 项，TypeScript/Vite 0.23.0 生产构建、静态资源逐字节一致、Python 编译、npm 高危依赖审计和差异格式检查均通过；测试只使用合成资料，生产业务写入、真实采集和模型调用均为 0。
- 本轮没有“正式发布”授权，未推进 `beijing-production`、未调用两端发布器、未重启生产服务。下一项唯一建议任务：所有者明确要求正式发布后，协调上线北京 0.10.0 与即时 AI 0.23.0，再用下一条自然发布的真实图文核对正文、原图顺序、评论和手机显示。
