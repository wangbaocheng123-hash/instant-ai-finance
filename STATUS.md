...
- 最后更新：2026-09-16 16:34（北京时间）
- 最新检查点：`MODEL_MR_IMAGE_POSTS_0_23_0_LIVE_PENDING_REAL_NOTE`。所有者已明确要求“正式发布”；功能提交 `50c8c2c1f77e034d7dd6d158a70f53e6fc692c5a` 已推送 `main` 与 `beijing-production`，即时 AI `0.23.0`、北京博主桥和模型下载器 `0.10.0+git.image-posts` 均已正式上线。模型先生主页现在同时识别 `/video/` 与 `/note/`，图文保存公开完整正文和全部有序 JPEG/PNG/WebP 原图，并经既有签名通道投影到主人私有手机原图库与 MCP 元数据。
- 新加坡固定发布器把生产从 `fb9d010` 安全快进到 `50c8c2c`，服务器 202 项回归通过；`instant-ai.service` active，回环/公网健康和 MCP `initialize` 均为 0.23.0，公网 Service Worker 摘要与生产仓库一致，未登录作品及图片 API 均返回 401。发布未迁移或改写既有 Git 外资料，也未触发 ASR、豆包或 AI。
- 北京统一发布完成后，博主与模型 accepted/deployed/current 均为 `50c8c2c`；博主 221 项、模型 18 项隔离测试通过，model/model-web/blogger 三服务 active，代码发布 timer 仍 disabled/inactive。模型数据库已原地升至 schema 5，`description`、`work_type`、`work_media` 均存在；16:31:59 至 16:32:15 的发布后真实首页扫描成功，没有触发下载。
- 北京到 GitHub 的直连在发布期间持续低速/超时；所有失败均停在 fetch，未切换组件或写失败 SHA。博主组件经同款沙箱预取后正常发布；模型组件最终通过只监听北京 `127.0.0.1`、随 SSH 会话销毁的新加坡回环 SOCKS 完成官方 origin 与精确 SHA 校验，再由固定模型发布器上线。没有修改 Git remote、DNS、Caddy、安全组、systemd、永久代理或业务数据。
- 当前生产模型库为 38 条视频、0 条图文、`work_media` 0 条；因此代码、schema、传输、私有接口和页面已经上线，但尚无自然发布的真实 `/note/` 可完成端到端样本验收。下一项唯一建议任务：等待模型先生下一条自然图文，核对公开正文、全部原图顺序、评论、主人认证、手机显示与新加坡投影；不得用合成数据冒充真实抖音验收。图片像素文字仍不做 OCR，也不混入公开正文。
