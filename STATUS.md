...
- 最后更新：2026-09-20 07:31（北京时间）
- 最新检查点：`MODEL_MR_VIDEO_STORAGE_AND_COMPACT_RELEASE_LIVE_VERIFIED`。所有者明确要求“正式发布”后，功能提交 `39da14f` 与结果基线 `d8d3a5a` 已进入 GitHub `main`；即时 AI 固定发布器把新加坡生产从 `50c8c2c` 安全快进到 `d8d3a5a`，服务器 207 项回归通过并上线 0.23.1。
...
`真机验收 0.23.1 并观察下一条自然新视频`：所有者在 iPhone 登录模型先生后，对同一部既有本地视频连续播放至结束，确认不再需要第二次或第三次点击；北京出现下一条自然视频后，再核对其半尺寸、H.264/AAC、快启、手机播放和新加坡传输均正常。不得为制造验收而重下、重编码历史视频或触发历史 ASR/豆包任务。
