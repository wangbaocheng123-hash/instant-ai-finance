...
class ProfileScanner:
        ...
        while time.monotonic() < deadline:
            ...
            if self.chrome.poll() is not None:
                raise ProfileScanError(
                    "专用浏览器配置正在被占用。请先关闭“模型下载器专用浏览器”，再重新检查。"
                )
    ...
    def scan(self, profile_url: str, timeout: float = 35) -> tuple[str, list[ProfileVideo]]:
        self._start()
        assert self.cdp
        ...
        while time.monotonic() < deadline:
            ...
            if len(by_id) >= MIN_STABLE_PROFILE_CARDS:
                creator = (last_snapshot.get("title") or "模型先生").split("的抖音")[0]
                videos = sorted(
